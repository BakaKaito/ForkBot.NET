...
namespace SysBot.Pokemon
{
    public static class SpecialRequests
    {
        ...
        public static SpecialTradeType CheckItemRequest(ref PK8 pk, PokeRoutineExecutor caller, PokeTradeDetail<PK8> detail, string TrainerName, SAV8SWSH sav)
        {
            ...
            if (pk.HeldItem >= 2 && pk.HeldItem <= 4) // ultra<>pokeball
            {
                switch (pk.HeldItem)
                {
                    case 2: //ultra
                        ...
                        caller.Log("Nickname and OT Changed Sucessfully");
                        break;
                    case 3: //great
                        pk.OT_Name = TrainerName;
                        caller.Log("OT Changed Sucessfully");
                        break;
                    case 4: //poke
                        pk.ClearNickname();
                        caller.Log("Nickname Changed Sucessfully");
                        break;
                }
            ...
            else if (pk.Nickname.Contains("pls"))
            {
                ...
                else
                {
                    ...
                    caller.Log("SSR Invalid Request");
            ...
            else if ((pk.HeldItem >= 30 && pk.HeldItem <= 32) || pk.HeldItem == 27 || pk.HeldItem == 28 || pk.HeldItem == 63) // fresh water/pop/lemonade <> full heal(27) <> revive(28) <> pokedoll(63)
            {
               if (pk.HeldItem == 27)
                    pk.IVs = new int[] { 31, 31, 31, 31, 31, 31 };
                caller.Log("IVs Changed Succesfully!");
                if (pk.HeldItem == 28)
                ...
                if (pk.HeldItem == 31)
                    pk.CurrentLevel = 100;
                caller.Log("Level Changed Succesfully!");
                if (pk.HeldItem == 32)
                {
                    pk.IVs = new int[] { 31, 31, 31, 31, 31, 31 };
                    pk.CurrentLevel = 100;
                    caller.Log("IVs & Level Changed Succesfully!");
                }
        ...
        private static PK8? LoadEvent(string v, SAV8SWSH sav)
        {
            ...
            pathwc = Path.Combine("wc", v + ".pcd");
            if (File.Exists(pathwc)) { wc = File.ReadAllBytes(pathwc); type = "pcd"; }
